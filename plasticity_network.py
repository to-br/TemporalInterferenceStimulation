    ...
    if delay_sd != 0:
        ...
        for idx, node in enumerate(nodes_A):
            nest.Connect(node, node_B, syn_spec={"weight": weight, 
                                                 ...
                                                 "tau_rec": tau_rec,  # recovery time from synaptic depression [ms]
                                                 "tau_fac": tau_fac,   # time constant for facilitation (off) [ms]
                                                 "tau_psc": tau_syn_ex}) # decay constant of PSCs (tau_inact in [2]) [ms]
