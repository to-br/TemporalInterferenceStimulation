...
def psd(spike_times, sim_time, bin_size=0.25, segments=16, f=1000):
...
def autocorr(spike_times, bin_size, sim_time, normalize=False, demean=True):
    spike_rate, spike_counts = find_spike_rate(spike_times, bin_size, sim_time)
    #spike_rate = np.asarray(spike_rate, dtype=float) # 
    if demean: # removing DC component to make sure the oscillations dominate
        spike_rate = spike_rate - spike_rate.mean()

    r = np.correlate(spike_rate, spike_rate, mode="full")   # length 2N-1
   # r = r[r.size//2:]                     # keep >= 0

    if normalize:
        r = r / r[0]                      # r[0] = energi/varians* N (avhengig av definisjon)
    return r

def psd_autocorr(spike_times, bin_size, sim_time, resolution, fmax=None, f=1000):
    """
    
    bin_size_ms: tidssteg i ms (samme som du brukte til spike_rate)
    """
    # r: autocorrelation for lags >= 0  (array, length N)
    r = autocorr(spike_times, bin_size, sim_time, normalize=False, demean=True)
    dt = bin_size / 1000  # sekunder
    print("dt", dt)
    fs = 1000 / bin_size
    print("fs:", fs)

    # FFT av autokorrelasjon (Wiener–Khinchin)
